...
export class Rings {
    ...
    create() {
        ...
        const box5 = BABYLON.MeshBuilder.CreatePlane("myPlane", {width: 30, height: 200}, this.scene);

        for (let theta = Math.PI / 2; theta < 2 * Math.PI + Math.PI / 2 - Math.PI / 50; theta += Math.PI / 50) {
            gtheta = theta;
            this.ring1SPS.addShape(box5, 1, { positionFunction: ring1PositionFunction });
        }
        ...
        for (let theta = Math.PI / 2; theta <= 2 * Math.PI + Math.PI / 2 ; theta += Math.PI / 100) {
                gtheta = theta;
            this.ring5SPS.addShape(box, 1, { positionFunction: ring5PositionFunction });
        }
